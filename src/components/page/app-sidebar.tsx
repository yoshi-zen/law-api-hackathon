 ...
 */
function MenuItemComponent({ item, level = 0 }: { item: MenuItem; level?: number }): JSX.Element {
    ...
    if (item.children && item.children.length > 0 && item.children.every(child => typeof child === "string")) {
        return (
            <>
                    ...
                    <SidebarMenuSub className="ml-4">
                        {item.children.map((child, index) => {
                            ...
                            // biome-ignore lint/style/noUselessElse: <explanation>
                                ...
                                return <MenuItemComponent key={`${child.tag}-${index}`} item={child} level={level + 1} />;
                            }
                        })}
                    </SidebarMenuSub>
                </CollapsibleContent>
...
export function AppSidebar() {
    ...
    for (const item of itemMock) {
        ...
        if (["TOCLabel","TOCChapter", "MainProvision", "SupplProvision"].includes(item.tag)) {
            break;
        }
    }
                    ...
                    <SidebarGroupContent>
                        <SidebarMenu>
                            {filteredItems.map((item, index) => (
                                <MenuItemComponent key={`${item.tag}-${index}`} item={item} />
                            ))}
                        </SidebarMenu>
                    </SidebarGroupContent>
                </SidebarGroup>
